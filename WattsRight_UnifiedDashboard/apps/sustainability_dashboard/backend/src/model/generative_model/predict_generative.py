...
# HuggingFace repo mapping for all known short model names
MODEL_REPOS = {
...
# The special test model skips real evaluation and uses bundled CSV data
TEST_MODEL_NAME = "pythia-1.4b-test"
_TEST_DATA_PATH = _DIR / "test_data" / "pythia-1.4b.csv"
...
def _load_metadata():
    """Load only metadata.json (no joblib). Safe for any numpy version."""
    global _metadata
...
def _load_artefacts():
    """Load GP model + scaler + metadata. Needed only for prediction."""
    global _gp_ppl, _scaler_ppl
    _load_metadata()
    ...
    # Fix numpy random state incompatibility across environments
    if hasattr(_gp_ppl, 'random_state'):
        import numpy as _np
        _gp_ppl.random_state = _np.random.RandomState(67)
...
def get_dropdown_models() -> list[str]:
    """Models shown in the frontend dropdown (val + test + test shortcut)."""
    return get_validation_models() + get_test_models() + [TEST_MODEL_NAME]


def get_hf_repo(name: str) -> str:
    """Map a short model name to its HuggingFace repo. Pass-through if unknown."""
    return MODEL_REPOS.get(name, name)


def is_test_model(name: str) -> bool:
    """Return True if the name is the special test model shortcut."""
    return name == TEST_MODEL_NAME


def load_test_model_data() -> dict[float, dict]:
    """
    Load the bundled pre-computed CSV for the test model.
    Returns dict mapping threshold (float) -> row dict.
    """
    result: dict[float, dict] = {}
    with open(_TEST_DATA_PATH, "r", encoding="utf-8") as f:
        reader = csv.DictReader(f)
        for row in reader:
            t = round(float(row["threshold"]), 1)
...
def predict_perplexity_curve(
    ...
    return {
        ...
        "knee_threshold": round(float(thresholds[knee_idx]), 1),
        "knee_perplexity": round(float(ppl_pred[knee_idx]), 2),
        "knee_flops_reduction": round(float(flops_actual[knee_idx]), 1),
    }


def build_result_from_real_data(
    real_perplexities: dict[float, float],
    flops_at_thresholds: dict[float, float],
) -> dict:
    """
    Build a surrogate-style result dict directly from real data (no GP needed).
    Used for the test model where we have all 101 thresholds measured.
    """
    thresholds = np.arange(0.0, 10.05, 0.1)
    ppl = np.array([real_perplexities.get(round(t, 1), 0.0) for t in thresholds])
    flops = np.array([flops_at_thresholds.get(round(t, 1), 0.0) for t in thresholds])

    pareto_idx = _find_pareto_front(ppl, flops)
    pareto_idx = pareto_idx[np.argsort(ppl[pareto_idx])]
    knee_in_pareto = _find_knee_point(ppl[pareto_idx], flops[pareto_idx])
    knee_idx = pareto_idx[knee_in_pareto]

    return {
        "thresholds": thresholds.tolist(),
        "perplexity": ppl.tolist(),
        "perplexity_std": [0.0] * len(thresholds),
        "flops_reduction_pct": flops.tolist(),
        "pareto_indices": pareto_idx.tolist(),
        "knee_threshold": round(float(thresholds[knee_idx]), 1),
        "knee_perplexity": round(float(ppl[knee_idx]), 2),
        "knee_flops_reduction": round(float(flops[knee_idx]), 1),
    }
